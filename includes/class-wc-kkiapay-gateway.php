...
class WC_Kkiapay_Gateway extends WC_Payment_Gateway
{
    ...
    {
        // Always return true since we now support multiple currencies
        return true;
    }
...
<?php
    ...
    private function setKkiapayConfig(WC_Order $order)
    {
        if (version_compare(WOOCOMMERCE_VERSION, '2.7.0', '>=')) {
            ...
            $this->kkiapay_config['amount'] = $amount;
    ...
    public function on_kkiapay_back()
    {
        ...
        if (isset($transaction_id)) {
            ...
            if ($response->status == STATUS::SUCCESS) {
                // Convert the response amount to store currency if needed
                $currency = get_woocommerce_currency();
            ...
            // Get order from order_id if it exists
    ...
    function on_kkiapay_webhook()
    {
        ...
        } elseif ($status == STATUS::FAILED && $order) {
            $this->handlePaymentFailed($order);
        }
        exit;
    }
